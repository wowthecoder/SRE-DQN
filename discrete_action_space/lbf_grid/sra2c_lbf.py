...
DEFAULT_SRA2C_SOLVER = "path_mcp_nplayer"
DEFAULT_SRA2C_OUTPUT_ROOT = Path("lbf_sra2c_runs")


@dataclass(frozen=True)
class Sra2cLbfHyperparams:
    agent: Sra2cConfig = field(
        default_factory=lambda: Sra2cConfig(
            actor_lr=3e-4,
            critic_lr=3e-4,
            value_lr=3e-4,
            gamma=0.99,
...
def sra2c_lbf_hyperparams(overrides=None):
    if isinstance(overrides, Sra2cLbfHyperparams):
        return overrides
    hp = Sra2cLbfHyperparams()
    if not overrides:
        return hp
    ...
    agent_field_names = {field.name for field in fields(Sra2cConfig)}
    for key in list(payload):
        if key in agent_field_names:
            agent_updates[key] = payload.pop(key)
    if payload:
        unknown = ", ".join(sorted(payload))
        raise KeyError(f"Unknown SR-A2C LBF hyperparameter section(s): {unknown}")
    if agent_updates:
        unknown = set(agent_updates) - agent_field_names
        if unknown:
            raise KeyError(
                "Unknown SR-A2C agent hyperparameter field(s): "
                + ", ".join(sorted(unknown))
            )
...
def train_lbf_sra2c_vectorized(
    *,
    n_episodes=500,
    solver_name=DEFAULT_SRA2C_SOLVER,
    epsilon_robust_initial=0.5,
    epsilon_schedule="constant",
    seed=BASE_SEED,
    output_root=DEFAULT_SRA2C_OUTPUT_ROOT,
    run_dir=None,
    ...
    if solver_name == DEFAULT_SRA2C_SOLVER and num_agents == 2:
        solver_name = "path_c_pool"
    elif solver_name == DEFAULT_SRA2C_SOLVER and num_agents > 2:
        solver_name = "path_mcp_nplayer_pool"

    run_name = f"sra2c_{_slugify(solver_name)}_eps{epsilon_robust_initial:g}_{epsilon_schedule}"
    if run_name_suffix:
        run_name = f"{run_name}__{run_name_suffix}"
    run_dir = Path(run_dir) if run_dir is not None else Path(output_root) / run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    agent = _make_sra2c_agent(
        state_dim=state_dim,
        actor_obs_dim=actor_obs_dim,
        num_agents=num_agents,
        num_actions=num_actions,
    ...
    stats = {
        ...
        "pairing": ["SR-A2C" for _ in range(num_agents)],
        "pair_label": "SR-A2C self-play",
        "pair_slug": run_name,
        "training_mode": "vectorized",
        ...
        "checkpoint_paths": {
            "best": str(run_dir / "shared_sra2c_best.pt"),
    ...
    if print_full_stats:
        print_stats_payload(stats, f"LBF SR-A2C vectorized - {solver_name}")
    print("\nReward Summary")
...
train_lbf_sra2c_vectorized_experiment = train_lbf_sra2c_vectorized
