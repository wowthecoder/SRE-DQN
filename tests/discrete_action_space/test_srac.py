import numpy as np
import pytest

from sre_solvers import SreSolveResult


class _FakeBatchSolver:
    name = "fake_srac_solver"

    def __init__(self):
        self.calls = 0
        self.shapes = []
        self.closed = False

    def solve_batch(self, q_tensors, epsilon, **_kwargs):
        del epsilon
        self.calls += 1
        results = []
        for q_tensor in q_tensors:
            q_tensor = np.asarray(q_tensor)
            self.shapes.append(q_tensor.shape)
            results.append(
                SreSolveResult(
                    policies=[
                        np.array([1.0, 0.0], dtype=np.float64),
                        np.array([0.0, 1.0], dtype=np.float64),
                    ],
                    solutions=[],
                    utilities_sr=[],
                    utilities_nominal=[],
                    success=True,
                )
            )
        return results

    def close(self):
        self.closed = True


def test_srac_action_sampling_respects_masks():
    torch = pytest.importorskip("torch")
    from srac import SracAgent, SracConfig

    del torch
    agent = SracAgent(
        SracConfig(
            state_dim=3,
            actor_obs_dim=2,
            num_agents=2,
            num_actions=2,
            epsilon_explore=1.0,
            use_gpu=False,
            sre_solver=_FakeBatchSolver(),
        )
    )
    try:
        for _ in range(20):
            actions = agent.act_joint(
                np.zeros(3, dtype=np.float32),
                np.zeros((2, 2), dtype=np.float32),
                action_masks=[
                    np.array([True, False]),
                    np.array([False, True]),
                ],
            )
            assert actions == [0, 1]
    finally:
        agent.close()


def test_srac_builds_critic_payoff_tensor_from_query_critic():
    pytest.importorskip("torch")
    from srac import SracAgent, SracConfig

    agent = SracAgent(
        SracConfig(
            state_dim=3,
            actor_obs_dim=2,
            num_agents=2,
            num_actions=3,
            use_gpu=False,
            sre_solver=_FakeBatchSolver(),
        )
    )
    try:
        q_tensor, action_indices = agent._payoff_game_from_critic(
            agent.critic,
            np.zeros(3, dtype=np.float32),
            [
                np.array([True, False, True]),
                np.array([False, True, True]),
            ],
        )
        assert q_tensor.shape == (2, 2, 2)
        assert [indices.tolist() for indices in action_indices] == [[0, 2], [1, 2]]
    finally:
        agent.close()


def test_srac_train_step_updates_critic_and_actor_from_sre_labels():
    pytest.importorskip("torch")
    from srac import SracAgent, SracConfig

    solver = _FakeBatchSolver()
    agent = SracAgent(
        SracConfig(
            state_dim=3,
            actor_obs_dim=2,
            num_agents=2,
            num_actions=2,
            batch_size=1,
            learning_starts=1,
            train_every=1,
            actor_update_every=1,
            epsilon_explore=0.0,
            use_gpu=False,
            sre_solver=solver,
            sre_target_value_mode="nominal",
        )
    )
    try:
        result = agent.update(
            state=np.zeros(3, dtype=np.float32),
            local_obs=np.zeros((2, 2), dtype=np.float32),
            joint_actions=[0, 1],
            joint_rewards=[1.0, 2.0],
            next_state=np.ones(3, dtype=np.float32),
            next_local_obs=np.ones((2, 2), dtype=np.float32),
            done=np.ones(2, dtype=np.float32),
            action_masks=[
                np.array([True, True]),
                np.array([True, True]),
            ],
            next_action_masks=None,
        )
        assert result is not None
        assert result["critic_loss"] is not None
        assert result["actor_loss"] is not None
        assert result["valid_critic_rows"] == 1
        assert result["valid_actor_rows"] == 1
        assert solver.calls >= 1
    finally:
        agent.close()
    assert solver.closed is True


def test_srac_target_value_mode_nominal_matches_policy_expectation():
    pytest.importorskip("torch")
    from srac import SracAgent, SracConfig

    agent = SracAgent(
        SracConfig(
            state_dim=3,
            actor_obs_dim=2,
            num_agents=2,
            num_actions=2,
            epsilon_robust=0.5,
            use_gpu=False,
            sre_solver=_FakeBatchSolver(),
            sre_target_value_mode="nominal",
        )
    )
    try:
        q_tensor = np.zeros((2, 2, 2), dtype=np.float32)
        q_tensor[:, :, 0] = np.array([[0.0, 2.0], [4.0, 6.0]], dtype=np.float32)
        q_tensor[:, :, 1] = 1.0
        policies = [
            np.array([0.25, 0.75], dtype=np.float32),
            np.array([0.5, 0.5], dtype=np.float32),
        ]
        values = agent._target_values(
            q_tensor,
            policies,
            [np.array([0, 1]), np.array([0, 1])],
        )
        assert values[0] == pytest.approx(4.0)
        assert values[1] == pytest.approx(1.0)
    finally:
        agent.close()
